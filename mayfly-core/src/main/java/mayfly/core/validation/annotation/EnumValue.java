...
/**
 * 枚举值校验，即限制参数值只能是指定枚举类中的值.
 * <p>可作用于枚举值如 {@code Integer}、{@code String}等基础类型，也可以作用于枚举类型,如:
 *
 * <pre>{@code
 * // 至少需要实现ValueEnum接口, 实现NameValueEnum接口错误提示时可附带枚举名称即描述
 * enum StatusEnum implements NameValueEnum<Integer> {
 *    ENABLE(1, "启用"),
 *    DISABLE(0, "禁用");
 *    .... 省略赋值与实现接口的相关方法
 * }
 *
 * // 限制Integer类型的status值只能为StatusEnum中指定value即 1和0
 * // 如果传其他值，则会错误提示"status枚举值错误，可选值为[1:启用, 0:禁用]"
 * @EnumValue(StatusEnum.class)
 * private Integer status;
 *
 * // 限制StatusEnum类型的status值只能为StatusEnum中的ENABLE
 * @EnumValue(value = StatusEnum.class, values = {"ENABLE"})
